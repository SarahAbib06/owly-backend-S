...
export const login = async (req, res) => {
  ...
  if (passwordHash.length < 8) return res.status(400).json({ message: 'Le mot de passe doit contenir au min 8 caractères' });
  ...
  try {
    ...
    const otp = generateOtp();
    const otpToken = jwt.sign({ userId: user._id, otp, type: 'inactivity' }, process.env.JWT_SECRET, { expiresIn: '10m' });
